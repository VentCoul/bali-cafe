...
Ви можете перенести код на сервер за допомогою Git (найкращий варіант) або скопіювати через SFTP (FileZilla/Cyberduck).

Якщо через Git (рекомендовано):
```bash
git clone https://github.com/ВАШ_ЛОГІН/bali.git
cd bali
```
