          ...
          <div className="flex items-center gap-6">
            ...
            {status === "loading" ? (
              ...
              <div className="relative group">
                ...
                <div className="absolute right-0 top-full w-48 bg-white rounded-xl shadow-lg border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right scale-95 group-hover:scale-100 overflow-hidden">
                  <div className="py-2">
                    {session.user.role === 'ADMIN' ? (
                      <>
                        <Link href="/admin/menu" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-[var(--color-bali-gold)] transition-colors">Меню сайту</Link>
                        <Link href="/admin/clients" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-[var(--color-bali-gold)] transition-colors">База клієнтів</Link>
                        <Link href="/api/poster/config" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-[var(--color-bali-gold)] transition-colors">Poster API</Link>
                      </>
                    ) : (
                      <>
                        <Link href="/profile" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-[var(--color-bali-gold)] transition-colors">Особистий кабінет</Link>
                      </>
                    )}
              ...
              <Link href={`/login?callbackUrl=${encodeURIComponent(pathname)}`} className="font-medium hover:text-[var(--color-bali-gold)] transition-colors">
                Увійти
              </Link>
            )}
