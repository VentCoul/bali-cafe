...
Телефония нужна ресторанам у которых есть свой call центр и которые принимают заказы на доставку. Интеграция должна упрощать ввод данных клиента в базу ресторана и делать работу оператора более персонализированой. Например, когда звонит уже известный номер, к клиенту можно обратиться по имени, сказать его последний адрес доставки и заказ.  
...
Кассу дополняют функциями при помощи методов [POS платформы](/docs/v3/pos/index). Чтобы начать разработку посмотрите раздел [Начало работы](/docs/v3/pos/start).

Разберем такой сценарий:
В заведении оператор принимает звонки на доставку. Клиент первый раз звонит, чтобы заказать пиццу в офис. Оператору call центра необходимо:
...
3. Во время входящего звонка, покажите всплывающее окно методом [Poster.interface.popup](/docs/v3/pos/interfaces/interface-popup). В этом окне нужно дать возможность: 
3.1 Заполнить контакты клиента и быстро создать новый заказ. 
3.2 Показать контакты клиента и быстро создать новый заказ.
