...
| workshops      | Список из id цехов на которые печатает принтер. `0` — означает печать чека заказа, все цеха возвращаются в поле [Poster.settings.workshops](/docs/v3/pos/settings). |
