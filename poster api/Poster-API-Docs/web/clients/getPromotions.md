...
const url = 'https://joinposter.com/api/clients.getPromotions
...
{  
   "response":[  
      {  
         ...
         "date_start":"2024-04-29 00:00:00",
      ...
      {  
         ...
         "date_start":"2024-04-30 00:00:00",
         "date_end":"0000-00-00 00:00:00",
         "auto_apply":"0"
      }
   ]
...
Внутри параметра `response` лежит массив, в каждом элементе которого есть:
...
Внутри параметра `params` лежит массив, в каждом элементе которого:
...
Внутри параметра `conditions` лежит массив, в каждом элементе которого:
...
Внутри параметра `periods` лежит массив, в каждом элементе которого:
...
Внутри параметра `discount_prices` лежит массив, в каждом элементе которого:
...
Внутри параметра `bonus_products` лежит массив, в каждом элементе которого:

| Параметр     | Описание                                                                                                                              |
|--------------|---------------------------------------------------------------------------------------------------------------------------------------|
| type         | Тип сущности: 1 — категория товаров, 2 — тех. карта либо товар, 3 — товар с модификаторами, 3 — тех. карта с модификаторами.          |
| id           | ID сущности                                                                                                                           |
| product_id   | Возвращается только если это товар с модификаторами. ID товара.                                             |
