...
Если знаете логин аккаунта, используйте:   
...
Параметр | Значение
-------- | --------
application_id | Id приложения в системе Poster, указан в настройках приложения в [аккаунте разработчика](https://dev.joinposter.com/login)
redirect_uri | URL на который пользователь направляется после первого шага авторизации. Должен точно совпадать с тем, что указан в настройках приложения на в [аккаунте разработчика](https://dev.joinposter.com/login).
...
Доступы приложения указываются в настройках приложения в [аккаунте разработчика](https://dev.joinposter.com/login). 
...
account | Логин аккаунта в системе Poster, который необходимо использовать для отправки всех последующих запросов к API Poster.
...
!> Тело запроса отправляйте в формате form-data
...
Параметр | Значение
-------- | -------- 
code | 32-символьный код, который получили на 2м шаге авторизации в GET параметре
application_id | ID приложения в системе Poster. Указан в настройках приложения в [аккаунте разработчика](/login).
application_secret | Секретный код приложения выданные при регистрации. Указан в настройках приложения в [аккаунте разработчика](/login).
redirect_uri | URL, на который пользователь направляется после первого шага авторизации. Должен точно совпадать с тем, что указан в настройках приложения в [аккаунте разработчика](/login).
...
Параметр | Значение
-------- | -------- 
access_token | Заветный токен, который открывает доступ к API. Время жизни токена 2 года.
...
Объект `user` содержит параметры: 
...
Объект `ownerInfo` содержит параметры: 
...
Объект `tariff` содержит параметры: 
